    ...
    <table class="min-w-full border-collapse border border-gray-400 mt-4">
        ...
        <tbody>
            @foreach ($unitkerjas as $unitkerja)
                <tr class="bg-gray-800 text-white"> {{-- Tambahkan class ini jika ingin baris gelap --}}
                    ...
                    <td class="py-2 px-4 border border-gray-300">
                        <flux:button :href="route('unitkerja.edit', $unitkerja)">Edit</flux:button>
                        <flux:button variant="danger" wire:click="delete({{ $unitkerja->id }})" wire:confirm="Are you sure?">Delete</flux:button>
                    </td>
                </tr>
            @endforeach
        </tbody>
