<div class="container mx-auto">
    ...
    <table class="min-w-full border-collapse border border-gray-400 mt-4">
        <thead>
            <tr class="text-left bg-gray-100 text-gray-800 font-semibold">
        ...
        <tbody>
            @foreach ($peminjamans as $p)
                <tr class="bg-gray-800 text-white">
                    <td class="py-2 px-4 border border-gray-300">{{ $p->id }}</td>
                    <td class="py-2 px-4 border border-gray-300">{{ $p->pegawai->nama }}</td>
                    <td class="py-2 px-4 border border-gray-300">{{ $p->ruang->nama }}</td>
                    <td class="py-2 px-4 border border-gray-300">{{ $p->tanggal }}</td>
                    <td class="py-2 px-4 border border-gray-300">{{ $p->jam_mulai }}</td>
                    <td class="py-2 px-4 border border-gray-300">{{ $p->jam_akhir }}</td>
                    <td class="py-2 px-4 border border-gray-300">{{ $p->keterangan ?? '-' }}</td>
                    <td class="py-2 px-4 border border-gray-300">
                        <flux:button :href="route('peminjaman.edit', $p)">Edit</flux:button>
                        <flux:button variant="danger" wire:click="delete({{ $p->id }})" wire:confirm="Are you sure?">Delete</flux:button>
                    </td>
                </tr>
            @endforeach
        </tbody>
